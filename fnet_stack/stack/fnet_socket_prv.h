...
typedef struct fnet_socket_prot_if
{
    ...
    int  (*prot_setsockopt)(fnet_socket_t *sk, int level, int optname, const void *optval, unsigned int optlen);           /* Protocol "setsockopt" function. */
    int  (*prot_getsockopt)(fnet_socket_t *sk, int level, int optname, void *optval, unsigned int *optlen);          /* Protocol "getsockopt" function. */
